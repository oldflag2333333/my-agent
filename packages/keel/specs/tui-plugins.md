...
- Author package entrypoint is `@keel-ai/plugin/tui`.
...
```json
{
  "$schema": "https://keel.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/keel-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@keel-ai/plugin/tui`.
- `@keel-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running Keel version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.keel` is absent, no check is performed (backward compatible).
...
- `keel plugin <module>` and TUI install both run install -> manifest read -> config patch.
- Alias: `keel plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.keel` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.keel`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.keel`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.keel` dir or `KEEL_CONFIG_DIR`, Keel installs `@keel-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@keel-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.keel/themes` area near the plugin config source.
...
- `--pure` / `KEEL_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.keel/plugins/tui-smoke.tsx`
- Local smoke config: `.keel/tui.json`
- Local smoke theme: `.keel/plugins/smoke-theme.json`
