...
export const DEPRECATED_PLUGIN_PACKAGES = ["keel-openai-codex-auth", "keel-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, keelVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(keelVersion) || semver.major(keelVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.keel
  if (typeof range !== "string") return
  if (!semver.satisfies(keelVersion, range)) {
    throw new Error(`Plugin requires keel ${range} but running ${keelVersion}`)
  }
}
