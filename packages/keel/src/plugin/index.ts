import type { Hooks, PluginInput, Plugin as PluginInstance, PluginModule } from "@keel-ai/plugin"
import { Config } from "../config/config"
import { Bus } from "../bus"
import { Log } from "../util/log"
import { createKeelClient } from "@keel-ai/sdk"
...
export namespace Plugin {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@keel/Plugin") {}
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createKeelClient({
            ...
            headers: Flag.KEEL_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.KEEL_SERVER_USERNAME ?? "keel"}:${Flag.KEEL_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
            fetch: ((req, init) => Server.Default().fetch(new Request(req, init))) as typeof fetch,
          })
          ...
          const plugins = Flag.KEEL_PURE ? [] : (cfg.plugin ?? [])
          if (Flag.KEEL_PURE && cfg.plugin?.length) {
