...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for keel.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal keel sessions
...
```bash
KEEL_ENABLE_QUESTION_TOOL=1 keel acp
```
...
- **Terminal Integration**: Full terminal support via keel's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | keel acp
```
...
ACP sessions map cleanly to keel's internal session model:
