...
import type { AssistantMessage, Event, KeelClient, SessionMessageResponse, ToolPart } from "@keel-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: KeelClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `keel auth login` in the terminal",
        name: "Login with keel",
        id: "keel-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Keel stores files internally as { type: "file", url, filename, mime }.
