...
import { NamedError } from "@keel-ai/util/error"
...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/keel"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "keel")
      default:
        return "/etc/keel"
    }
  }
  ...
  export function managedConfigDir() {
    return process.env.KEEL_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }
  ...
  export async function installDependencies(dir: string, input?: InstallInput) {
    ...
    await BunProc.run(
      ...
      if (err instanceof Process.RunFailedError) {
        ...
        if (Flag.KEEL_STRICT_CONFIG_DEPS) {
  ...
  export async function needsInstall(dir: string) {
    ...
    const mod = path.join(dir, "node_modules", "@keel-ai", "plugin")
    ...
    const depVersion = dependencies["@keel-ai/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      if (!online()) return false
      const stale = await PackageRegistry.isOutdated("@keel-ai/plugin", depVersion, dir)
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.keel/command/", "/.keel/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.keel/agent/", "/.keel/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: keel.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for keel serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://keel.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://keel.ai/docs/agents"),
      provider: z
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@keel/Config") {}

  function globalConfigFile() {
    const candidates = ["keel.jsonc", "keel.json", "config.json"].map((file) => path.join(Global.Path.config, file))
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          const normalized = (() => {
            ...
            delete copy.tui
            log.warn("tui keys in keel config are deprecated; move them to tui.json", { path: source })
            return copy
          })()
          ...
          if (parsed.success) {
            if (!parsed.data.$schema && isFile) {
              parsed.data.$schema = "https://keel.ai/config.json"
            ...
            yield* Effect.promise(() =>
              import(pathToFileURL(legacy).href, { with: { type: "toml" } })
                .then(async (mod) => {
                  ...
                  result["$schema"] = "https://keel.ai/config.json"
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          for (const [key, value] of Object.entries(auth)) {
            if (value.type === "wellknown") {
              ...
              log.debug("fetching remote config", { url: `${url}/.well-known/keel` })
              const response = yield* Effect.promise(() => fetch(`${url}/.well-known/keel`))
              if (!response.ok) {
                throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
              }
              const wellknown = (yield* Effect.promise(() => response.json())) as any
              const remoteConfig = wellknown.config ?? {}
              if (!remoteConfig.$schema) remoteConfig.$schema = "https://keel.ai/config.json"
              result = mergeConfigConcatArrays(
                result,
                yield* loadConfig(JSON.stringify(remoteConfig), {
                  dir: path.dirname(`${url}/.well-known/keel`),
                  source: `${url}/.well-known/keel`,
                }),
              )
          ...
          if (Flag.KEEL_CONFIG) {
            result = mergeConfigConcatArrays(result, yield* loadFile(Flag.KEEL_CONFIG))
            log.debug("loaded custom config", { path: Flag.KEEL_CONFIG })
          }

          if (!Flag.KEEL_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("keel", ctx.directory, ctx.worktree),
            )) {
          ...
          if (Flag.KEEL_CONFIG_DIR) {
            log.debug("loading config from KEEL_CONFIG_DIR", { path: Flag.KEEL_CONFIG_DIR })
          }

          const deps: Promise<void>[] = []

          for (const dir of unique(directories)) {
            if (dir.endsWith(".keel") || dir === Flag.KEEL_CONFIG_DIR) {
              for (const file of ["keel.jsonc", "keel.json"]) {
          ...
          if (process.env.KEEL_CONFIG_CONTENT) {
            result = mergeConfigConcatArrays(
              result,
              yield* loadConfig(process.env.KEEL_CONFIG_CONTENT, {
                dir: ctx.directory,
                source: "KEEL_CONFIG_CONTENT",
              }),
            )
            log.debug("loaded custom config from KEEL_CONFIG_CONTENT")
          }
          ...
          if (active?.active_org_id) {
            yield* Effect.gen(function* () {
              ...
              if (token) {
                process.env["KEEL_CONSOLE_TOKEN"] = token
                Env.set("KEEL_CONSOLE_TOKEN", token)
              }
          ...
          if (Flag.KEEL_PERMISSION) {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.KEEL_PERMISSION))
          }
          ...
          if (Flag.KEEL_DISABLE_AUTOCOMPACT) {
            result.compaction = { ...result.compaction, auto: false }
          }
          if (Flag.KEEL_DISABLE_PRUNE) {
