...
const TUI_SCHEMA_URL = "https://keel.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from keel.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const files = await serverFiles(input)
...
async function serverFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.KEEL_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("keel", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "keel")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "keel"))
  }
  if (Flag.KEEL_CONFIG) files.push(Flag.KEEL_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "keel"))
