  ...
  export const ControlPlaneRoutes = (opts?: { cors?: string[] }): Hono => {
    ...
    return app
      ...
      .use((c, next) => {
        ...
        const password = Flag.KEEL_SERVER_PASSWORD
        if (!password) return next()
        const username = Flag.KEEL_SERVER_USERNAME ?? "keel"
      ...
      .use(
        cors({
          ...
          origin(input) {
            ...
            // *.keel.ai (https only, adjust if needed)
            if (/^https:\/\/([a-z0-9-]+\.)*keel\.ai$/.test(input)) {
              return input
            }
