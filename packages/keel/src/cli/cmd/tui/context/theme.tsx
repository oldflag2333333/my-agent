...
import keel from "./theme/keel.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@keel-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "keel")
        draft.active = typeof active === "string" ? active : "keel"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.keel, store.mode)
    })
