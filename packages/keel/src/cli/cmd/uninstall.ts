...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall keel and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @keel-ai/cli",
      pnpm: "pnpm uninstall -g @keel-ai/cli",
      bun: "bun remove -g @keel-ai/cli",
      yarn: "yarn global remove @keel-ai/cli",
      brew: "brew uninstall keel",
      choco: "choco uninstall keel",
      scoop: "scoop uninstall keel",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "keel", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Keel!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# keel") || content.includes(".keel/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".keel/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".keel/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".keel"))
    ) {
      continue
    }
