...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run keel",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found keel session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["keel", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting keel...")
        UI.println()

        const keelArgs = sessionId ? ["-s", sessionId] : []
        const keelProcess = Process.spawn(["keel", ...keelArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await keelProcess.exited
        if (code !== 0) throw new Error(`keel exited with code ${code}`)
      },
    })
  },
})
