...
Replace runtime Bun shell template-tag usage in `packages/keel/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/keel/src` is removed except:
