...
const managedConfigDir = process.env.KEEL_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "keel.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "keel.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://keel.ai/config.json",
      snapshot: false,
    })
...
test("ignores legacy tui keys in keel config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://keel.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "keel.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["KEEL_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:KEEL_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["KEEL_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["KEEL_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "keel.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .keel directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const keelDir = path.join(dir, ".keel")
      await fs.mkdir(keelDir, { recursive: true })
      const agentDir = path.join(keelDir, "agent")
...
test("loads agents from .keel/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const keelDir = path.join(dir, ".keel")
      await fs.mkdir(keelDir, { recursive: true })

      const agentsDir = path.join(keelDir, "agents")
...
test("loads commands from .keel/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const keelDir = path.join(dir, ".keel")
      await fs.mkdir(keelDir, { recursive: true })

      const commandDir = path.join(keelDir, "command")
...
test("loads commands from .keel/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const keelDir = path.join(dir, ".keel")
      await fs.mkdir(keelDir, { recursive: true })

      const commandsDir = path.join(keelDir, "commands")
...
test("does not try to install dependencies in read-only KEEL_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.KEEL_CONFIG_DIR
  process.env.KEEL_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.KEEL_CONFIG_DIR
    else process.env.KEEL_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable KEEL_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const run = spyOn(BunProc, "run").mockImplementation(async (_cmd, opts) => {
    const mod = path.join(opts?.cwd ?? "", "node_modules", "@keel-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@keel-ai/plugin", version: "1.0.0" }),
    )
    return {
      code: 0,
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "keel.json"),
        JSON.stringify({ $schema: "https://keel.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .keel config
      const projectDir = path.join(dir, "project")
      const keelDir = path.join(projectDir, ".keel")
      await fs.mkdir(keelDir, { recursive: true })
      ...
      // Local .keel config with different plugins
      await Filesystem.write(
        path.join(keelDir, "keel.json"),
        JSON.stringify({
          $schema: "https://keel.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .keel config with some overlapping plugins
      await Filesystem.write(
        path.join(keelDir, "keel.json"),
        JSON.stringify({
          $schema: "https://keel.ai/config.json",
...
// Note: preload.ts sets KEEL_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .keel config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .keel directory config enables it
      const keelDir = path.join(dir, ".keel")
      await fs.mkdir(keelDir, { recursive: true })
      await Filesystem.write(
        path.join(keelDir, "keel.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/keel")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/keel")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "keel.json")
    expect(await Config.resolvePluginSpec("oh-my-keel@2.4.3", file)).toBe("oh-my-keel@2.4.3")
...
describe("deduplicatePlugins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-keel@2.4.3", "file:///project/.keel/plugin/oh-my-keel.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.keel/plugin/demo.ts", "file:///project/.keel/plugin/demo.ts"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result).toEqual(["file:///project/.keel/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const keelDir = path.join(projectDir, ".keel")
        const pluginDir = path.join(keelDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("KEEL_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["KEEL_DISABLE_PROJECT_CONFIG"]
    process.env["KEEL_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["KEEL_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["KEEL_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .keel/ directories when flag is set", async () => {
    const originalEnv = process.env["KEEL_DISABLE_PROJECT_CONFIG"]
    process.env["KEEL_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .keel directory with a command
          const keelDir = path.join(dir, ".keel", "command")
          await fs.mkdir(keelDir, { recursive: true })
          await Filesystem.write(path.join(keelDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .keel should NOT be in directories list
          const hasProjectKeel = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectKeel).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["KEEL_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KEEL_CONFIG_DIR"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["KEEL_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["KEEL_CONFIG_DIR"]
      } else {
        process.env["KEEL_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("KEEL_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["KEEL_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KEEL_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["KEEL_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["KEEL_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from KEEL_CONFIG_DIR, not project
...
describe("KEEL_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in KEEL_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KEEL_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["KEEL_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://keel.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["KEEL_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["KEEL_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in KEEL_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KEEL_CONFIG_CONTENT"]
