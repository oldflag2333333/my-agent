...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.KEEL_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.KEEL_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.KEEL_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.KEEL_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}
