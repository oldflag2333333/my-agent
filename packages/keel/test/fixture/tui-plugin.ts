import { createKeelClient } from "@keel-ai/sdk/v2"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  let selected = opts.theme?.selected ?? "keel"
  const key = {
  ...
  return {
    ...
    tuiConfig: (opts.tuiConfig ?? {}) as HostPluginApi["tuiConfig"],
    kv: {
      get: kvGet,
      set(name, value) {
        kv[name] = value
